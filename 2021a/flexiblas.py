...
class EB_FlexiBLAS(CMakeMake):
    ...
    def sanity_check_step(self):
        ...
        if 'INTEGER8=ON' in self.cfg['configopts']:
            suffix = '64'
        else:
            suffix = ''
        ...
        # libraries in lib/
        top_libs = ['libflexiblas%s%s.%s' % (suffix, x, shlib_ext) for x in ('', '_api', '_mgmt')]
        ...
        libs.extend(os.path.join('lib', 'flexiblas%s' % suffix, lf) for lf in lower_libs)

        custom_paths = {
            'files': [os.path.join('bin', 'flexiblas%s' % suffix), os.path.join('etc', 'flexiblas%src' % suffix)] + libs,
            'dirs': [os.path.join('etc', 'flexiblas%src.d' % suffix), os.path.join('share', 'man')],
        }

        custom_commands = [
            "flexiblas%s --help" % suffix,
            "flexiblas%s list" % suffix,
        ]

        # make sure that each BLAS library is supported by FlexiBLAS by checking output of 'flexiblas list'
        for blas_lib in self.blas_libs:
            custom_commands.append("flexiblas%s list | grep %s" % (suffix, blas_lib.upper()))
